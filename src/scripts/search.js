!function ($) {
    "use strict";
    var _okItems = {}, _requesting = false;
    var OKSearch = function (element, options) {
        ...
        this.shown = false
        this.listen()
        this.searchFor = {minister: {url: 'http://www.vaadat-sarim.org.il/ministers.json', title: 'שרים', render: function (item) {
            var el = $('<li class="search-option"><a></a></li>');
            el.find('a').attr('href', '#/' + item.url).text(item.name);//.append($('<img>').attr('src', item.img_url));
            return el;
        }}, bill: {url: 'http://www.vaadat-sarim.org.il/bills.json', title: 'הצעות חוק', render: function (item) {
            var el = $('<li class="search-option"><a></a></li>');
            el.find('a').attr('href', '#/' + item.url).text(item.name);
            return el;
        }}}
    }
    OKSearch.prototype = {constructor: OKSearch, sourceItems: function () {
        var that = this;
        if (_requesting || !$.isEmptyObject(_okItems))return;
        _requesting = true;
        var params = $.map(this.searchFor, function (value) {
            return $.get(value.url)
        })
        $.when.apply(this, params).done(function () {
            var count = 0, res = arguments;
        ...
        var val = $el.attr('data-value'), url = $el.find('a').attr('href');
        ...
        var that = this, elements = new Array();
        $.each(items, function (key, values) {
            if (values.length) {
                ...
                $.each(values, function (idx, item) {
                    var i = that.searchFor[key].render(item), highlighted = that.highlighter(item.name), $a = i.find('a'), $img = $a.find('img');
        ...
        var active = this.$menu.find('.active').removeClass('active'), next = active.nextAll('li:not(.nav-header)')
        if (!next.length) {
        ...
        var active = this.$menu.find('.active').removeClass('active'), prev = active.prevAll('li:not(.nav-header)')
        if (!prev.length) {
    ...
    $.fn.oksearch = function (option) {
        return this.each(function () {
            var $this = $(this), data = $this.data('oksearch'), options = typeof option == 'object' && option
            if (!data)$this.data('oksearch', (data = new OKSearch(this, options)))
            if (typeof option == 'string')data[option]()
        })
    }
    $.fn.oksearch.defaults = {source: null, items: 5, menu: '<ul class="oksearch dropdown-menu nav nav-list"></ul>', minLength: 1}
